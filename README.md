...
- **Full formatting.** Replies render Telegram's real formatting — **bold**, *italic*, ~~strikethrough~~, inline `code`, fenced code blocks with syntax highlighting, links, and blockquotes — converted straight from Claude's Markdown and chunked to fit Telegram's message-length limit. A Markdown pipe table (`| a | b |` / `|---|---|`) is rendered as an aligned monospace grid, since Telegram's HTML mode has no real `<table>` tag.
